...
class BootService:
    ...
    def install_grub(self, root_mount: Path, target_device: str) -> None:
        try:
            self.chroot.run_in_chroot(root_mount, ["grub-install", "--target=i386-pc", target_device])
            self.chroot.run_in_chroot(root_mount, ["grub-install", "--target=x86_64-efi", "--efi-directory=/boot/efi", "--bootloader-id=OYOPORT"])
            self.chroot.run_in_chroot(root_mount, ["update-grub"])
        except Exception as exc:
            raise AppError("E501", f"grub 設定失敗: {exc}") from exc

    def update_initramfs(self, root_mount: Path) -> None:
        try:
            self.chroot.run_in_chroot(root_mount, ["update-initramfs", "-u"])
        except Exception as exc:
